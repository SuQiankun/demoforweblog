...
(function (BH_UTILS, undefined) {
  ...
  BH_UTILS.fxss = function (value) {
    var warp = $("<div></div>");
    if (value instanceof jQuery) {
      return warp.append(value).html();
    } else {
      return warp.text(String(value)).html();
    }
  }
  ...
  BH_UTILS.uxss = function (value) {
    ...
    if (value instanceof jQuery) {
      ...
      return warp.html(String(value)).text();
    }
  }
   ...
   * @param {String} - 待翻译的内容格式为  {key}[|{defaultText},[...{params1},[...{params2}]]]
  ...
  BH_UTILS.i18n = function (content) {
    ...
    result = $.i18n(key);
    //判断匹配到的值是否含有{}
    var checkTemp = result.match(/(\{\d+\}?)/g);
    if (checkTemp !== null) {
